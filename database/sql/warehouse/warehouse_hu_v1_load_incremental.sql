...
-- Lee watermark desde analytics.wh_etl_watermark (process_name='hu_index_v1').
...
WITH wm AS (
    SELECT COALESCE(
        (SELECT last_max_write_date FROM analytics.wh_etl_watermark WHERE process_name = 'hu_index_v1'),
...
WHERE r.write_date > (SELECT watermark_ts FROM wm);
