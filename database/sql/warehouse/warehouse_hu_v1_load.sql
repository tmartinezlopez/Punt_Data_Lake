...
anchor_data AS (
    SELECT
        ...
        g.in_progress_nodes,
        g.canceled_nodes,
        g.sum_allocated_hours,
        g.sum_effective_hours,
        g.sum_remaining_hours,
        g.sum_total_hours_spent,
        g.avg_progress,
...
full_text AS (
    SELECT
        ...
        jsonb_build_object(
            ...
            'root_id', ad.root_id,
            'node_count', ad.node_count,
            'max_depth', ad.max_depth,
            'min_create_date', ad.min_create_date,
            'max_write_date', ad.max_write_date,
            'embedded_at', NOW()
        ) AS meta,
        LEFT(
            concat(
                '[META] group_id=', ad.group_id,
                ' project_id=', COALESCE(ad.project_id::text, 'N/A'),
                ' partner_id=', COALESCE(ad.partner_id::text, 'N/A'),
                ' root_id=', COALESCE(ad.root_id::text, 'N/A'),
                ' node_count=', ad.node_count,
                ' max_depth=', ad.max_depth,
                ...
                COALESCE((
                    SELECT string_agg(
                        concat(
                            'hu_id=', n1.node_id,
                            ' depth=', COALESCE(n1.depth::text, '0'),
                            ' state=', COALESCE(n1.state, 'N/A'),
                            ' active=', COALESCE(n1.active::text, 'N/A'),
                            ' progress=', COALESCE(n1.progress::text, '0'),
                            E'\n',
                            COALESCE(n1.description_clean, ''),
                            E'\n---\n'
                        ),
...
solution_text AS (
    SELECT
        ...
        jsonb_build_object(
            ...
            'project_id', ad.project_id,
            'partner_id', ad.partner_id,
            'done_nodes', ad.done_nodes,
            'in_progress_nodes', ad.in_progress_nodes,
            'canceled_nodes', ad.canceled_nodes,
            'max_write_date', ad.max_write_date,
            'embedded_at', NOW()
        ) AS meta,
        LEFT(
                ...
                COALESCE((
                    SELECT string_agg(
                        concat(
                            ...
                            ' write_date=', COALESCE(n1.write_date::text, 'N/A'),
                            E'\n',
                            COALESCE(n1.description_clean, ''),
                            E'\n---\n'
                        ),
...
desc_text AS (
    SELECT
        ...
        NOW() AS embedded_at,
        CASE
            WHEN length(COALESCE(ad.anchor_description, '')) >= 220 THEN 'anchor_only'
            ELSE 'anchor_plus_children'
        END AS description_source,
        jsonb_build_object(
            'group_id', ad.group_id,
            'anchor_task_id', ad.group_id,
            'project_id', ad.project_id,
            'partner_id', ad.partner_id,
...
SELECT group_id, 'full'::text, embedding_text, project_id, partner_id, embedded_at, meta, NOW(), NOW()
FROM full_text
UNION ALL
SELECT group_id, 'solution'::text, embedding_text, project_id, partner_id, embedded_at, meta, NOW(), NOW()
FROM solution_text
UNION ALL
SELECT group_id, 'hu_description'::text, embedding_text, project_id, partner_id, embedded_at, meta, NOW(), NOW()
FROM desc_text;
